...
import { CreateLoanCycleDto } from './dto/create-loancycle.dto';
...
export class LoanCycleService {
  ...
  async createLoanCycle(createLoanCycleDto: CreateLoanCycleDto) {
    const { groupId, recipientId, isCurrent, startDate, endDate, isActive } =
      createLoanCycleDto;
    ...
    // Create the loan cycle
    const loanCycle = await this.prisma.loanCycle.create({
      data: {
        group: { connect: { id: groupId } },
        recipient: { connect: { pk: recipientId } },
        isCurrent,
        startDate,
        endDate,
        isActive,
  ...
  // async createLoanCycle(payload: CreateLoanCycleDto): Promise<LoanCycle> {
